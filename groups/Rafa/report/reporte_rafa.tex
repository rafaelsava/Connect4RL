...
El agente final, \textbf{RafaImproved}, usa \textit{trial-based online policy improvement} (TBOPI). En cada turno ejecuta una búsqueda local con UCB desde el estado actual; esa búsqueda empieza con una Q-table global aprendida por self-play. Se distingue de otros enfoques del grupo porque no depende solo de una heurística fija ni solo de búsqueda online: combina aprendizaje offline persistente con mejora online por turno. La memoria no reemplaza el razonamiento online: funciona como un \textit{prior} que guía la exploración hacia acciones útiles.
